-- ============================================================
-- SPIIS – opdatering 9: lyn-opdateringer + sidste bestillingstider
-- Kør ÉN gang i Supabase → SQL Editor → Run.
-- (Indeholder ALT fra opdatering 5-8 – kør bare denne éne.)
--
-- 1) Realtime: nye bestillinger og bookinger lander i admin-appen
--    ØJEBLIKKELIGT – uden at vente og uden at genindlæse.
-- 2) Sidste bestillingstid: to-go senest kl. 19:30 og spis her
--    senest kl. 20:30 (kan ændres i admin → Åbningstider).
-- ============================================================

-- fluebenet på arrangementer (fra opdatering 8 – gør intet, hvis den findes)
alter table public.bookings add column if not exists block_orders boolean not null default true;

-- meld tabellerne til realtime, så admin får besked med det samme
do $$
begin
  begin
